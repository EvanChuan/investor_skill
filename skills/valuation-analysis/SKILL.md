...
description: >
  個股估值分析與安全邊際計算，透過相對估值（P/E、P/B、EV/EBITDA、PEG）、絕對估值（DCF、DDM）與資產估值三種方法，產出保守/中性/樂觀三情境目標價區間與安全邊際百分比。

  當使用者詢問以下問題時請使用本 skill：這支股票現在貴不貴？合理價位在哪裡？安全邊際多少？目標價怎麼估？本益比合理嗎？DCF 怎麼算？現在股價反映了什麼預期？何時是好的買入價格？適合承接第三階段個股基本面分析後執行，或在使用者直接要求估值任何股票時獨立使用。
version: 1.0.0
author: Evan
license: Proprietary
tags:
